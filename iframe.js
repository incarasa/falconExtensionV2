...
mejorarBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (data.texto_importante && data.texto_importante.trim() !== "") {
      const textoImportante = data.texto_importante.trim()
        .replace(/^\s*\*\*\s*$/gm, "")
        .replace(/^\s*[-*]\s*$/gm, "")
        .replace(/^\s*$/gm, "");
    
      const verRecomendaciones = document.createElement("a");
      verRecomendaciones.textContent = "🔎 Ver recomendaciones";
      verRecomendaciones.href = "#";
      verRecomendaciones.style.display = "inline-block";
      verRecomendaciones.style.marginTop = "10px";
      verRecomendaciones.style.color = "#0d47a1";
      verRecomendaciones.style.fontWeight = "bold";
      verRecomendaciones.style.textDecoration = "underline";
      verRecomendaciones.onclick = () => {
        const blob = new Blob([`
          <html>
            <head><title>Recomendaciones Médicas</title></head>
            <body style="font-family: sans-serif; padding: 20px;">
              <h1>IMPORTANTE</h1>
              <pre style="white-space: pre-wrap; font-size: 16px;">${textoImportante}</pre>
            </body>
          </html>
        `], { type: "text/html" });
      ...
      mensajeImportanteDiv.innerHTML = "";
      mensajeImportanteDiv.appendChild(verRecomendaciones);
